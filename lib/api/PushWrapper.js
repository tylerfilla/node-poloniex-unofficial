...
function PushWrapper() {
    ...
    this._connection.onopen = (session) => {
        ...
        for (let i = 0; i < this._cxnCallbacksOpen.length; i++) {
    ...
    this._connection.onclose = (reason, details) => {
        ...
        for (let i = 0; i < this._cxnCallbacksClose.length; i++) {
...
function connect(api, callback) {
    ...
    if (api._connection.isConnected) {
        ...
        let onClose = function(reason, details) {
            // Clean up
            if (doCleanup) {
                doCleanup();
            }
...
function subscribe(api, feed, callback) {
    ...
    connect(api, (err) => {
        if (err) {
            ...
            let callbackProxy = (args, kwargs, details) => {
